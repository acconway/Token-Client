...
exports.send = function(params) {
	...
	App.LOG("App.API.send Sending " + params.method + " request to URL " + baseURL + params.path);
	App.LOG("App.API.send Sending payload " + JSON.stringify(params.data));

	var xhr = createHTTPClient(params)
	
	var url = (baseURL + params.path);
	
	try {

		xhr.open(params.method, url);
		xhr.setRequestHeader("Content-Type", "application/json");
		xhr.send(params.method == "POST" ? (params.data ? params.data : null) : null);
